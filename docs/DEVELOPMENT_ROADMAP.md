...
- Boss TTK tuning from harness evidence (June 2026): replaced the blanket `BOSS_HP_MULTIPLIER = 4.0` with per-boss HP (Sentinel 62k / shield 4.4k, Hydra 100k per lobe, Void Prism 700k) and added `AURA_BOSS_DAMAGE_MULT = 25` so Halo/Barrier/Interference are a real boss archetype (their crowd-tuned ~100-130 flat DPS could never finish five-figure HP pools). Result: Sentinel 44.5/10.8/24.0 s and Hydra 40.9/21.4/43.8 s for balanced/beam/close on-curve builds (band 20-45 s); Void Prism intentionally stays the longest fight (22.5 s beam, ~74 s balanced, ~108 s close in the 120 s window) because killing it is optional. Known design debt: beam-count multiplication (Split×Mirror) makes beam the dominant single-target archetype; revisit with diminishing returns vs single targets if it warps builds.
